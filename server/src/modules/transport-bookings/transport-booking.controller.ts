...
export const TransportBookingController = {
  ...
  async getById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!item) return res.status(404).json({ message: 'Transport booking not found' });
  ...
  async create(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(201).json(successResponse(item, 'Transport booking created successfully'));
    } catch (err) {
      next(err);
    }
  },
  ...
  async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.json(successResponse(item, 'Transport booking updated successfully'));
    } catch (err) {
      next(err);
    }
  },
